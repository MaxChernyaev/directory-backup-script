...
create_backup() {
    if [[ -n "$1" && -n "$2" ]]; then # проверяем что оба аргумента были введены
        ...
        if [ -d $1 ]; then # проверяем существует ли архивируемый объект
            ...
            if [[ $(dpkg -s tar 2>/dev/null | grep "Status: install ok installed") == "" ]]; then # проверка установлен ли пакет tar
                ...
                tar -czf $ARCHIVE_NAME.tar.gz $WHAT # нужно ли использовать -P, чтобы убрать вывод "tar: Removing leading `/' from member names"?
                
                if [[ $(date +%m.%d) == "12.31" ]]; then # сегодня 31 декабря
                    WHERE=$WHERE/yearly
                elif [[ $(date +%d) == "01" ]]; then # сегодня 1 день месяца
                    WHERE=$WHERE/monthly
                elif [[ $(date +%A) == "Sunday" || $(date +%A) == "Воскресение" ]]; then # сегодня воскресение
                    WHERE=$WHERE/weekly
                else # сегодня любой другой день
                    WHERE=$WHERE/daily
                fi
                mkdir -p $WHERE
                mv $ARCHIVE_NAME.tar.gz $WHERE
...
clear_backup_folder() {
    if [[ $(ls -A $1/yearly | wc -l) > $YEARLY_FILE_LIMIT ]]; then
    ...
    if [[ $(ls -A $1/monthly | wc -l) > $MONTHLY_FILE_LIMIT ]]; then
    ...
    if [[ $(ls -A $1/weekly | wc -l) > $WEEKLY_FILE_LIMIT ]]; then
    ...
    if [[ $(ls -A $1/daily | wc -l) > $DAILY_FILE_LIMIT ]]; then
